...
(defn non-ranging-moves[board location rules]
  (let [ [x y] location
    ...
    (map (fn[loc2] [location loc2])
         (remove (fn[loc2] (occupied-by-enemy? board loc2 color))
                 (filter on-board? 
                         (map (fn[rule] (rule x y)) rules))
                 ))))
    ...
    (fn[[loc piece :as z]]
      (let [
        ...
        (cond (:ranging piece-info)
              (ranging-moves board loc (:rules piece-info)) 
              (is-pawn? piece)
              (pawn-moves board loc) 
              true
              (non-ranging-moves board loc (:rules piece-info)) 
              ))) (filter (fn[[_ piece]]
...
(defn controlled-by[board color]
  ...
  (set (map second (moves board color))))
...
(defn test-king-moves[]
  (is (= '([[0 0] [1 1]] [[0 0] [1 0]] [[0 0] [0 1]])
         (moves (string->board "k") :black))
      )
  (is (= '([[0 0] [1 1]] [[0 0] [0 1]])
         (moves (string->board "kN") :black)))
  )
(defn test-non-ranging-moves[]
  (is (= 
        '([[0 0] [2 1]])
        (non-ranging-moves (string->board "n") [0 0] knight-rules)
        ))
  (is (= 
...
(defn test-controlled-by[]
  ;; (moves (string->board "k") :black)
  (is (= #{[0 1][1 1][1 0]}
         (controlled-by (string->board "k") :black)
         )
      (is (= #{[0 1][1 1]}
             (controlled-by (string->board "kn") :black)
             )
          )))
;; (moves (string->board "kn") :black)
(defn test-knight-moves[]
  (is (= 8 
         (count (moves (string->board "  n\n   \n   ") :black))
